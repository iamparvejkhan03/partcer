...
const OrderDetails = () => {
    ...
    // Load conversations and find/create conversation with learner
    useEffect(() => {
        if (!socket || !isConnected || !order) return;

        const learnerId = order.studentId?._id;
        if (!learnerId) return;
        ...
        const handleConversationsList = (convs) => {
            ...
            const existingConv = convs.find(conv =>
                conv.participants.some(p => p._id === learnerId)
            );

            if (existingConv) {
                setConversation(existingConv);
            } else {
                socket.emit('conversation:getOrCreate', { receiverId: learnerId });
            }
            setLoading(false);
        };
    ...
    const handleSendMessage = (content, attachments = []) => {
        ...
        const receiverId = order.studentId?._id;
    ...
    const handleMarkAsDelivered = async () => {
        ...
        try {
            ...
            if (response.data?.success) {
                ...
                if (conversation) {
                    socket.emit('message:send', {
                        receiverId: order.studentId?._id,
                        content: `📦 Order has been marked as delivered. Please check your order details and confirm completion.`,
                        attachments: []
                    });
                }
            }
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-gray-500">
                                                                {onlineUsers?.includes(order.studentId?._id) ? 'Online' : 'Offline'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                ...
                                                >
                                                    {messageLoading ? (
                                                        ...
                                                        <div className="text-center py-20">
                                                            ...
                                                            <p className="text-sm text-gray-400">Start a conversation with your student</p>
                                                        </div>
                                                    ) : (
                                                        (() => {
                                                                                ...
                                                                                <div className={`flex max-w-[70%] ${isSender ? 'flex-row-reverse' : 'flex-row'}`}>
                                                                                    {!isSender && showAvatar && (
                                                                                        <div className="flex-shrink-0 mr-2">
                                                                                            <img
                                                                                                src={msg.sender?.profileImage || 'https://via.placeholder.com/32'}
