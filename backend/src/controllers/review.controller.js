 ...
 * @access  Private (Student or Mentor who was part of the order)
 */
export const addOrderReview = asyncHandler(async (req, res) => {
    ...
    // Find the order - NO POPULATE needed here
    ...
    const isStudent = order.studentId.toString() === userId.toString();
    const isMentor = order.mentorId.toString() === userId.toString();
    ...
    const reviewee = isStudent ? order.mentorId : order.studentId;
    const revieweeRole = isStudent ? "mentor" : "student";
...
export const getOrderReviews = asyncHandler(async (req, res) => {
    ...
    const isParticipant =
        order.studentId.toString() === userId.toString() ||
        order.mentorId.toString() === userId.toString();
    ...
    const reviews = await Review.getOrderReviews(orderId);
...
/**
 * @desc    Get reviews for a user (as reviewee - reviews they received)
...
export const getMyReviews = asyncHandler(async (req, res) => {
    ...
    const reviews = await Review.getReviewsByUser(userId, null, {
        limit: parseInt(limit),
        skip: (parseInt(page) - 1) * parseInt(limit),
    });
...
export const markHelpful = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, { helpfulCount: review.helpful.count, isHelpful: true }, "Marked as helpful"),
        );
});
...
export const unmarkHelpful = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, { helpfulCount: review.helpful.count, isHelpful: false }, "Removed helpful mark"),
        );
});
...
export const flagReview = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, null, "Review flagged successfully"));
});
...
export const adminGetReviewStats = asyncHandler(async (req, res) => {
    ...
    const stats = await Review.getPlatformStats(dateFilter);
